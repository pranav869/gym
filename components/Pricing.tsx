...
const WHATSAPP_PT = `https://wa.me/918838250227?text=${encodeURIComponent('Hi, I want to know more about Personal Training at APEX GYM!')}`
    ...
    <section id="pricing" className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            ...
            return (
              <motion.div
                ...
                className={`relative rounded-2xl p-7 border transition-colors duration-300 ${
                  plan.highlight
                    ? 'bg-gradient-to-b from-orange-50 to-white border-orange-500/60 shadow-2xl shadow-orange-500/10'
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-5">
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${plan.highlight ? 'bg-orange-500' : 'bg-gray-100'}`}>
                    <Icon className={`w-5 h-5 ${plan.highlight ? 'text-white' : 'text-gray-500'}`} />
                  </div>
                  <div>
                    <h3 className="text-gray-900 font-black text-xl">{plan.name} Plan</h3>
                ...
                <div className="mb-6">
                  <div className="flex items-baseline gap-1">
                    <span className={`text-2xl font-black ${plan.highlight ? 'text-orange-400' : 'text-gray-500'}`}>
                      Rs.
                    </span>
                    <span className={`text-6xl font-black leading-none ${plan.highlight ? 'text-gradient' : 'text-gray-900'}`}>
                ...
                <ul className="space-y-3">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-center gap-3">
                      <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${plan.highlight ? 'bg-orange-500/20' : 'bg-gray-100'}`}>
                        <Check className={`w-3 h-3 ${plan.highlight ? 'text-orange-400' : 'text-gray-500'}`} />
                      </div>
        ...
        >
          ...
          {/* Card */}
          <motion.div
            whileHover={{ y: -6, boxShadow: '0 24px 70px rgba(249,115,22,0.15)' }}
            className="relative rounded-2xl border border-orange-500/40 bg-gradient-to-br from-orange-50 via-white to-white p-7 overflow-hidden"
          >
            ...
            <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-8 items-center pt-4">
              <div>
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    <h3 className="text-gray-900 font-black text-2xl">Personal Training</h3>
                    <p className="text-gray-500 text-xs mt-0.5">1-on-1 with a dedicated trainer</p>
                  </div>
                </div>
                ...
                <div className="mb-6">
                  ...
                  <p className="text-gray-500 text-xs mt-1.5">Includes all gym access · No extra charges</p>
                </div>
              ...
              <div>
                <p className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-4">What&apos;s included</p>
                <ul className="space-y-3">
                  {ptFeatures.map((f) => (
                    <li key={f} className="flex items-center gap-3">
