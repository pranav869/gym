    ...
    <>
      ...
      >
        ...
        <motion.div
          ...
          style={{
            backgroundColor: useTransform(bgOpacity,    (v) => `rgba(255,255,255,${v * 0.92})`),
            borderBottom:     useTransform(borderAlpha,  (v) => `1px solid rgba(249,115,22,${v * 0.4})`),
            boxShadow:        useTransform(shadowAlpha,  (v) => `0 4px 24px rgba(0,0,0,${v * 0.08})`),
          }}
        />
            ...
            <nav className="hidden md:flex items-center gap-8">
              {links.map((link) => (
                <motion.a
                  ...
                  href={link.href}
                  whileHover={{ y: -1 }}
                  className="text-gray-700 hover:text-orange-500 text-sm font-medium transition-colors duration-200 relative group"
                >
                  {link.label}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="fixed top-16 left-0 right-0 z-40 backdrop-blur-2xl bg-white/95 border-t border-gray-200/60 overflow-hidden"
          >
            <div className="max-w-7xl mx-auto px-4 py-6 flex flex-col gap-4">
              {links.map((link, i) => (
                <motion.a
                  ...
                  className="text-gray-700 hover:text-orange-500 text-lg font-semibold py-2 border-b border-gray-200/50 transition-colors"
                >
                  {link.label}
                </motion.a>
              ))}
